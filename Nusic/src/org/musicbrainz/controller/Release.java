...
public class Release extends Controller {
	...
	private boolean needsLookUp(ReleaseIncludesWs2 inc) {
		...
		return (getRelease() == null || super.needsLookUp(inc) || inc.isLabel()
				|| inc.isReleaseGroups() || inc.isArtistCredits()
				|| inc.isMedia() || inc.isRecordings() || inc.isDiscids());
	}
	...
	public ReleaseWs2 lookUp(String id) throws MBWS2Exception {
		...
		if (needsLookUp(inc)) {
			...
			if (transit == null)
				return null;
			if (getRelease() == null || !getRelease().equals(transit)) // release
																		// is
																		// changed.
			{
				setEntity(transit);
