...
public abstract class Controller extends DomainsWs2 {
	...
	protected IncludesWs2 getIncrementalInc(IncludesWs2 inc) {
		...
		if (getIncludes().isArtistRelations()
				&& !getIncluded().isArtistRelations())
			inc.setArtistRelations(true);
		if (getIncludes().isLabelRelations()
				&& !getIncluded().isLabelRelations())
			inc.setLabelRelations(true);
		if (getIncludes().isReleaseRelations()
				&& !getIncluded().isReleaseRelations())
			inc.setReleaseRelations(true);
		if (getIncludes().isReleaseGroupRelations()
				&& !getIncluded().isReleaseGroupRelations())
			inc.setReleaseGroupRelations(true);
		if (getIncludes().isRecordingRelations()
				&& !getIncluded().isRecordingRelations())
		...
		if (getIncludes().isRecordingLevelRelations()
				&& !getIncluded().isRecordingLevelRelations())
			inc.setRecordingLevelRelations(true);
		if (getIncludes().isWorkLevelRelations()
				&& !getIncluded().isWorkLevelRelations())
	...
	protected boolean needsLookUp(IncludesWs2 inc) {
		...
		return (inc.isArtistRelations() || inc.isLabelRelations()
				|| inc.isReleaseRelations() || inc.isReleaseGroupRelations()
				|| inc.isRecordingRelations() || inc.isWorkRelations()
				|| inc.isUrlRelations() || inc.isTags() || inc.isRatings()
				|| inc.isUserTags() || inc.isUserRatings());
	}

	protected void updateEntity(EntityWs2 entity, EntityWs2 transit,
			IncludesWs2 inc) throws MBWS2Exception {
	...
	private void updateUserTags(EntityWs2 entity, EntityWs2 transit,
			IncludesWs2 inc) {
	...
	private void updateRatings(EntityWs2 entity, EntityWs2 transit,
			IncludesWs2 inc) {
	...
	private void updateUserRatings(EntityWs2 entity, EntityWs2 transit,
			IncludesWs2 inc) {
	...
	private void updateRelations(EntityWs2 entity, EntityWs2 transit,
			IncludesWs2 inc) throws MBWS2Exception {
	...
	public final void postUserRatings() throws MBWS2Exception {
		...
		UserRatingSubmissionWs2 query = new UserRatingSubmissionWs2(
				getQueryWs());
		query.addEntity(entity);
		Metadata md = query.submit();
		// You could also test the metadata.message if is OK or throw an
