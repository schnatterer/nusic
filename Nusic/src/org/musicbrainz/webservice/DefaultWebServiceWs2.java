...
/**
 * A default abstract web service implementation that provides common properties
 * of a web service client that can be extended.
 */
public abstract class DefaultWebServiceWs2 extends DomainsWs2 implements
		WebService {
	 ...
	 * 
	 * @see DefaultWebServiceWs1#makeUrl(String, String, Map, Map, String,
	 *      String)
	 */
	...
	/*
	 * All POST requests require authentication. You should authenticate using
	 * HTTP Digest, use the same username and password you use to access the
	 * main http://musicbrainz.org website. The realm is "musicbrainz.org".
	 * 
	 ...
	 * submitting data. This has to be the application's name and version
	 * number, not that of a client library (client libraries should use HTTP's
	 * User-Agent header). The recommended format is "application-version",
	 * where version does not contain a - character.
	 */
	/**
	 ...
	 *            XML parser used to consume the response stream
	 * @param writer
	 *            XML used to produce the body of POST requests
	 */
	 ...
	 */
	protected abstract Metadata doGet(String url) throws WebServiceException,
			MbXMLException;
	 ...
	 * @param url
	 * @param data
	 *            Input stream of the data to post
	 * @throws WebServiceException
	 */
	protected abstract Metadata doPost(String url, Metadata md)
	 ...
	 * @throws WebServiceException
	 */
	protected abstract Metadata doPut(String url) throws WebServiceException,
			MbXMLException;
	 ...
	 * @throws WebServiceException
	 */
	protected abstract Metadata doDelete(String url)
			throws WebServiceException, MbXMLException;

	/*
	 * (non-Javadoc)
	 * 
	 * @see org.musicbrainz.webservice.WebService#get(java.lang.String,
	 * java.lang.String, java.util.List, java.util.Map)
	 */
	public Metadata get(String entity, String id, List<String> includeParams,
			Map<String, String> filterParams) throws WebServiceException,
			MbXMLException {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.musicbrainz.webservice.WebService#post(java.lang.String,
	 * java.lang.String, java.io.InputStream)
	 */
	public Metadata post(Metadata metadata) throws WebServiceException,
			MbXMLException {
		if (metadata == null || metadata.getSubmissionWs2() == null)
			throw new SubmissionException("Empty Submission not allowed");

		String url = this.makeURLforPost(metadata.getSubmissionWs2()
				.getSubmissionType(), getClient());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.musicbrainz.webservice.WebService#put(java.lang.String,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.musicbrainz.webservice.WebService#delete(java.lang.String,
	 * java.lang.String, List<String>)
	 */
	...
	/**
	 * Constructs a URL that can be used to query the web service. The url is
	 * made up of the protocol, host, port, version, type, path and parameters.
	 * 
	 * @param entity
	 *            The entity (i.e. type, e.g. 'artist') the request is targeting
	 * @param id
	 *            The id of the entity
	 * @param includeParams
	 *            A list containing values for the 'inc' parameter (can be null)
	 * @param filterParams
	 *            Additional parameters depending on the entity (can be null)
	 * 
	...
	protected String makeURLforPost(String submissionType, String client) {
		...
		// append path
		url.append(PATHPREFIX).append("/").append(WS_VERSION).append("/")
				.append(submissionType).append("?client=").append(client);
	 ...
	 * @return An URL as String
	 */
	protected String makeURL(String entity, String id,
			List<String> includeParams, Map<String, String> filterParams) {
		...
		// append path
		url.append(PATHPREFIX).append("/").append(WS_VERSION).append("/")
				.append(entity).append("/").append(id);
		...
		while (it.hasNext()) {
			...
			try {
				url.append(e.getKey()).append("=")
						.append(URLEncoder.encode(e.getValue(), URL_ENCODING))
						.append("&");
			} catch (UnsupportedEncodingException ex) {
				log.error(
						"Internal Error: Could not encode url parameter "
								+ e.getKey(), ex);
			}
		}
